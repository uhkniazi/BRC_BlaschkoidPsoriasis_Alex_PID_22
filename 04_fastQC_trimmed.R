...
g_did
dfSample = dbGetQuery(db, "select * from Sample where idData=43;")
...
q = paste0('select File.*, Sample.id as sid, Sample.* from File, Sample where Sample.idData=43 AND Sample.id=File.idSample AND File.type="fastq"', ';')
...
setwd(gcswd)
n = make.names(paste('CFastqQualityBatch trimmed data id 43 alex rds'))
...
df = data.frame(idData=g_did, name=n, type='rds', location='~/Data/MetaData/', comment='trimmed FASTQ quality checks on Alex human data')
...
## read lengths from 3 experiments are of different sizes so sort this issue 
